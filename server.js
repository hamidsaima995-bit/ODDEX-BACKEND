...
export default function OddexVibe() {
  ...
  return (
    <div style={{ fontFamily:"'JetBrains Mono','Courier New',monospace", background:"#040409", color:"#ddd",
      ...
      {activeNews && (
        <div style={{background:"linear-gradient(90deg,"+(activeNews.impact>0?"#00ff8818":"#ff446618")+",transparent)",
          borderBottom:"1px solid "+(activeNews.impact>0?"#00ff8833":"#ff446633"),
          padding:"5px clamp(10px,3vw,16px)",display:"flex",alignItems:"center",gap:8,flexShrink:0,overflow:"hidden"}}>
          <span style={{fontSize:"0.6rem",fontWeight:700,letterSpacing:"0.08em",color:activeNews.impact>0?"#00ff88":"#ff4466",flexShrink:0}}>
            📰 LIVE
          </span>
          <span style={{fontSize:"clamp(0.6rem,2vw,0.68rem)",color:"#ccc",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",flex:1}}>
            {activeNews.headline}
          </span>
          <span style={{fontSize:"0.64rem",fontWeight:700,color:activeNews.impact>0?"#00ff88":"#ff4466",flexShrink:0}}>
            {activeNews.symbol} {activeNews.impact>0?"▲":"▼"}{Math.abs(activeNews.impact)}%
          </span>
        </div>
      )}
